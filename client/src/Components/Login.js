...
function Login({ setUser }) {
    ...
    return (
        <div style={{background: "linear-gradient(140.6deg, #052331 55%, #004c3f 100.2%)"}} className="flex min-h-screen w-screen items-center justify-center text-white">
            <form onSubmit={handleSubmit} className="md:flex h-64 text-md flex flex-col bg-gray-700 border border-gray-900 p-6 rounded-lg justify-center md:w-1/2 lg:w-1/3">
                <label className="mb-2 text-lg font-medium">Username or Email:</label>
                <input placeholder="Please enter your username or email address" onChange={(e) => setUsername(e.target.value)} value={username} type="text" id="username" name="username" className="border border-gray-600 rounded-lg text-black px-2 py-1 mb-4"></input>
                <label className="mb-2 text-lg font-medium">Password:</label>
                <input placeholder="Please enter your password" onChange={(e) => setPassword(e.target.value)} value={password} type="password" className="border border-gray-600 rounded-lg text-black px-2 py-1 mb-4" name="password"></input>
